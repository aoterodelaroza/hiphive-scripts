...
temperatures = list(range(0,3001,1)) # extended temperature list
...
f0 = xx[0,1] / 4.184 / 627.50947 ## zero-point energy in Ha
t = xx[1:,0] ## temperature in K (skip 0 K)
s = xx[1:,3] / 1000 / 4.184 / 627.50947 ## entropy in Ha/K (skip 0 K)
...
res = scipy.optimize.least_squares(lsqr_residuals_debye,1000,bounds=(0,np.inf),ftol=1e-12,xtol=None,gtol=None,verbose=2)
td = res.x[0]
...
res = scipy.optimize.least_squares(lsqr_residuals_extdebye,[td] + npoly * [0],method='lm',ftol=1e-15,xtol=1e-15,gtol=1e-15,verbose=2)
